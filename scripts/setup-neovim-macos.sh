...
echo "======================================"
echo "  Neovim IDE Setup Script (macOS)"
echo "======================================"
echo ""
...
DEPS_TO_INSTALL=""
command -v rg &> /dev/null || DEPS_TO_INSTALL="$DEPS_TO_INSTALL ripgrep"
command -v fd &> /dev/null || DEPS_TO_INSTALL="$DEPS_TO_INSTALL fd"
command -v node &> /dev/null || DEPS_TO_INSTALL="$DEPS_TO_INSTALL node"
...
if ! command -v black &> /dev/null && ! [ -f "$PYTHON_BIN_PATH/black" ]; then
  pip3 install --user --break-system-packages black flake8
else
  echo "✓ black and flake8 already installed"
fi

if ! echo "$PATH" | grep -q "$PYTHON_BIN_PATH"; then
  echo ""
  echo "⚠️  Adding Python bin to PATH..."
  SHELL_RC=""
  ...
  if [ -n "$SHELL_RC" ] && [ -f "$SHELL_RC" ]; then
    if ! grep -q "Library/Python/$PYTHON_VERSION/bin" "$SHELL_RC"; then
